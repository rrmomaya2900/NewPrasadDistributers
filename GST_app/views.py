    ...
    if request.method == 'POST':
        try :
            ...
            # return redirect(f'/success/?gst_number={gst_number}')
                ...
                print(form.errors.as_json())  # 👈 JSON is easier to read
        except Exception as e:
            import traceback
            print("Unexpected error in form submission:", str(e))
            traceback.print_exc()  # 
...
def generate_updated_pdf(request, gst_number):
    ...
    # Save the URL to the instance and persist it
        ...
        # Return PDF response
        response = HttpResponse(pdf_bytes, content_type='application/pdf')
        response['Content-Disposition'] = f'inline; filename="acknowledgement_{gst_number}_{instance.year_of_filing}.pdf"'
        return response
